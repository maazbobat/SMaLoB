...
const AdminVendors = () => {
  ...
  const handleSaveVendor = async () => {
    try {
      const url = editingVendor
        ? `http://localhost:3001/api/admin/vendors/${editingVendor._id}`
        : "http://localhost:3001/api/admin/vendors";
  ...
  const handleDeleteVendor = async (vendorId) => {
    ...
    try {
      await axios.delete(`http://localhost:3001/api/admin/vendors/${vendorId}`, {
        headers: { Authorization: `Bearer ${user.token}` },
      });
      fetchVendors();
    } catch (error) {
